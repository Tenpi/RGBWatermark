...
const NetworkShifter: React.FunctionComponent = (props) => {
    const [network, setNetwork] = useState("")
    const [networkName, setNetworkName] = useState("")
    const [destFolder, setDestFolder] = useState("")
    const [fHover, setFhover] = useState(false)
    const [error, setError] = useState("")
    const [shift, setShift] = useState("0.001")
    ...
    const shifter = async () => {
        if (network && networkName && destFolder) {
            let newName = `${path.basename(networkName, path.extname(networkName))}_shifted${path.extname(networkName)}`
            const dest = path.join(destFolder, newName)
            try {
                await ipcRenderer.invoke("shift-network", network, dest, Number(shift), Number(probability))
            } catch (err) {
                console.error(err)
    ...
    useEffect(() => {
        const savedShift = localStorage.getItem("networkShift")
        if (savedShift) setShift(savedShift)
        const savedProbability = localStorage.getItem("networkProbability")
        if (savedProbability) setProbability(savedProbability)
    }, [])

    useEffect(() => {
        localStorage.setItem("networkShift", shift)
        localStorage.setItem("networkProbability", probability)
    }, [shift, probability])
    ...
    return (
        <div className="network-randomizer">
            <div className="network-upload-container">
                ...
                <div className="network-row">
                    <span className="network-text">Shift:</span>
                    <input className="network-input" onChange={(event) => setShift(event.target.value)} value={shift}></input>
                </div>
                <div className="network-row">
                    <span className="network-text">Probability:</span>
                    <input className="network-input" onChange={(event) => setProbability(event.target.value)} value={probability} style={{width: "40px"}}></input>
                </div>
                <div className="network-row">
                    <button className="network-button" onClick={reset}>
                        <span className="button-hover">
                ...
                <div className="network-row">
                    <span className="network-text">Network shifter takes in a network (.ckpt, .safetensors, .pt, .bin, .onnx) and shifts the value of the tensors. The probability controls the chance of a tensor being shifted. A high enough shift will mess up the output.</span>
                </div>
                <div className="network-row">
                ...
                <div className="network-row">
                    <button className="network-button-3" onClick={shifter} style={{marginTop: "5px"}}>
                        <span className="button-hover">
                            <span className="button-text">Shift</span>
                        </span>
                    </button>
                </div>
            </div>
